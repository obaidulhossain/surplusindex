...
def availableLeads(request):
    ...
    leads_queryset = Foreclosure.objects.exclude(purchased_by=user).exclude(sale_status="CANCELLED" or "ACTIVE").exclude(surplus_status="No Surplus").exclude(surplus_status=None)
